...
# Ceci est effectivevment accepté par python
# mais pour la question suivante il va falloir écrire l'algorithme pour trouver un motif...
...
*Réponse:*
#+begin_src python
def correction_QCM(reponses_etudiants,corrige):
    resultats = {}
    ...
    for etudiant in reponses_etudiants.keys():
        ...
        for reponse in reponses_etudiants[etudiant].keys():
            if reponses_etudiants[etudiant][reponse] == corrige[reponse]:
                points += 3
            else:
                points -= 1
